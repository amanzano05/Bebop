...
class Tracker:
    ...
    def calculateSpeed(self, cX, cY):
        ...
        speedZ=((((abs(cY)-self.offset)*(self.topSpeed-self.speedOffset))/(self.centerWidth-self.offset))+self.speedOffset)
        if (cX<0):signX=-1
        else: signX=1
        if (cY<0):signY=-1
        else: signY=1
        return speedZ*signY, speedR*signX
    ...
    def camera_callback(self, data):
        try:
            ...
            if (not self.tracking):
                ...
                self.correct_Ystart=int(self.centerHeight)-self.offset
                self.correct_Yend=int(self.centerHeight)+self.offset
               ...
               #draw bounding box
                if ok:
                    ...
                    if (abs(coordinateX)>self.offset and abs(coordinateY)>self.offset):
                        speedZ, speedR=self.calculateSpeed(coordinateX, coordinateY)
                        self.twist=Twist(Vector3(0.0,0.0,speedZ), Vector3(0.0,0.0,speedR))
                        cv2.putText(self.stream, ("Speed Z: "+str("%.2f" % speedZ)+" R:"+str("%.2f" % speedR)), (10,60), cv2.FONT_HERSHEY_SIMPLEX, 0.75,(50,170,50),2)
                        self.pub_move.publish(self.twist)
                        self.centered=False
                        ...
                        self.pub_move.publish(self.twist)
                        self.centered=False
                    elif(abs(coordinateY)>self.offset):
                        speedZ,_=self.calculateSpeed(coordinateX, coordinateY)
                        speedR=0
                        cv2.putText(self.stream, ("Speed Z: "+str("%.2f" % speedZ)+" R:"+str("%.2f" % speedR)), (10,60), cv2.FONT_HERSHEY_SIMPLEX, 0.75,(50,170,50),2)
                        self.twist=Twist(Vector3(0.0,0.0,speedZ), Vector3(0.0,0.0,speedR))
                        self.pub_move.publish(self.twist)
